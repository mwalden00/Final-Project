...
def get_lighting(normal, view, ambient, lights, symbols, reflect ):
    ...
    for light in lights:
        # N+=1
        # print('light {}'.format(N))
        n = normal[:]
        normalize(n)
        normalize(light[LOCATION])
        normalize(view)

        d[RED] += calculate_diffuse(light, r, n)[RED]
        s[RED] += calculate_specular(light, r, view, n)[RED]
        d[GREEN] += calculate_diffuse(light, r, n)[GREEN]
        s[GREEN] += calculate_specular(light, r, view, n)[GREEN]
        d[BLUE] += calculate_diffuse(light, r, n)[BLUE]
        s[BLUE] += calculate_specular(light, r, view, n)[BLUE]
        #print(d)
